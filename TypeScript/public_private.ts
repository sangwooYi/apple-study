...
class UserH {
  private static x: number = 10;  // private이므로 외부에서 접근 불가, static이므로 클래스 내부에서도 this 사용 불가
  public static y: number = 20;  //  public이므로 외부 접근은 가능하나 static이므로 클래스명.필드 이렇게 접근해야 함
...
class Square {
  ...
  draw() {
    ...
    const randH: number = Math.floor(Math.random() * 400);
    const randW: number = Math.floor(Math.random() * 400);
